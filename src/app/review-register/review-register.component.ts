...
export class ReviewRegisterComponent implements OnInit {
  ...
  constructor(private service: ProviderService, private snackBar : MatSnackBar) {}
  ...
  openModal(providerID: number) {
    this.service.getProviderByID(providerID).subscribe((data) => {
      this.modalData = data;
  ...
  approveButton(providerID: number) {
    this.service.approveProvider(providerID).subscribe(
      (response) => {
        console.log('Status updated successfully', response);
        this.closeModal(); // Menutup modal setelah berhasil approve
        this.getListProvider(); // Memperbarui daftar
        this.showNotification('Provider approved successfully', 'Close', 'custom-snackbar-success');
      },
      (error) => {
  ...
  submitRejection() {
    ...
    if (this.currentProviderID !== null) {
      this.service.rejectProvider(this.currentProviderID, this.rejectionMessage).subscribe(
        (response) => {
          console.log('Reject account successfully', response);
          this.closeRejectModal();
          this.getListProvider();
          this.closeModal();
          this.showNotification('Provider rejected successfully', 'Close', 'custom-snackbar-error');
        },
        (error) => {
  ...
  getListProvider(): void {
    this.service.getListProvider().subscribe((data) => {
