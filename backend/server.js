  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!imageServiceFiles || imageServiceFiles.length < 3) {
        return res
          .status(400)
          .json({ message: "You must upload at least 3 service images." });
      }
...
app.get("/pending-services", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error retrieving pending services", error });
  }
});
...
app.put("/approve/:providerID", async (req, res) => {
  try {
    ...
    res.json({
      message: "Provider status updated successfully and service created",
      updatedProvider,
      newService,
    });
  } catch (error) {
...
app.put("/service/approve/:id", async (req, res) => {
  try {
    ...
    res.json({
      message: "Service status updated to approved successfully",
      updatedService,
    });
  } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (req.files["productImages"]) {
        const newImagePaths = req.files["productImages"].map(
          (file) => file.path
      ...
      const existingService = await Service.findById(
        req.params.id,
        "productImages"
      );
      if (!existingService) {
      ...
      if (req.body.imagesToRemove) {
        const imagesToRemove =
          req.body.imagesToRemove instanceof Array
...
app.put(
  "/api/services/update/tourGuide/:id",
  upload.array("productImages", 10),
  async (req, res) => {
    // console.log(req.body);
    try {
      const { productName, productDescription, productPrice, location } =
        req.body;
